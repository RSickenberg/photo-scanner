...
- **Amended (2026-09-26), at the user's request to save NAS space:** TIFFs use deflate *with the horizontal predictor* (standard TIFF, read by Preview and every other reader). On real files it's lossless and as fast, and cuts Scans by 23% and masters by 28%. A Calibration keeps only what it's computed from: the full-resolution grey image (dust) and the colour image at detection size (background), as a two-page TIFF. That's 11.5 MB instead of 38.5 MB, since an empty glass is mostly noise that no lossless compression shrinks, and the result is identical (tested). Calibrations kept as whole Scans still load. `photoscan compact` rewrites older TIFFs locally, and on the NAS for pruned files, replacing each only once the new file reads back identical and keeping its modification time. JPEG XL lossless was rejected: another 10–15% smaller but an extra library, 6 s per Scan to write, and Preview can't open it. Deleting old Calibrations was rejected: `recut` needs them.
...
- **Amended (2026-09-25):** the "Scan's own lid colour" used by that second test was the most common colour along the Scan's edge. With four Prints filling the glass, only 6% of the edge was still lid, and it came out as a photo's near-black (L 37 vs 236): dark areas inside the photos read as lid and four Prints were cut into 8 pieces. Now, when the edge colour disagrees with the calibrated lid colour (by more than twice the cut-off), the lid colour is taken where the Scan still looks like the empty glass (within twice the cut-off of the reference). Otherwise the edge colour is kept: using the new estimate everywhere was tried and rejected, because it put caption strips (L 229-231) right at the cut-off and cut them short on real Scans. Known limit: without a calibration (skipped, or pruned before a `recut`) there's nothing to check the edge colour against, and such a layout still fails.
