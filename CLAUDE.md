...
`photoscan`: a macOS CLI that drives a Canon CanoScan LiDE 400 through SANE,
cuts each Print out of a multi-photo Scan into its own file (TIFF + JPEG), and
backs the result up to a NAS. Built for digitising a family photo archive.
Domain vocabulary (Print, Scan, Extract, Session, Label, Backup) lives in
`CONTEXT.md`: use those terms in code and docs.

## Stack

Python ≥3.12 (uv, typer, OpenCV, tifffile, Pillow), SANE `scanimage` from
Homebrew for scanner access (ADR 0001). A `package.json` exists **only** for
release tooling (release-it); the version source is `VERSION`.
...
| Run the CLI | `make run ARGS="session 'Label'"` |
| Run tests | `make test` |
...
- Never delete files under the user's `output_dir` or NAS folder outside `photoscan prune`'s verified path.
